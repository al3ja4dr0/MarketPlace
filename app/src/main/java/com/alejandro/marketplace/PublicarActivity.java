...
public class PublicarActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // INICIO MODIFICACIÓN SEMANA 4: try-catch en la llamada de la galería (mínimo, peroútil)
    ...
    private boolean validarCampos() {
        ...
        if (TextUtils.isEmpty(precio) || !precio.matches("^[0-9]+([.,][0-9]{1,2})?$")) {
            etPrecio.setError("Ingresa un precio válido (solo números y opcionalmente dosdecimales).");
            return false;
        }
        ...
        if (TextUtils.isEmpty(descripcion) || descripcion.length() < 10) {
            etDescripcion.setError("La descripción es obligatoria y debe tener al menos 10caracteres.");
            return false;
        }
        return true;
    }
    private void simularPublicacion() {
        String nombre = etNombre.getText().toString().trim();
        String direccion = etDireccion.getText().toString().trim(); // Usar la dirección en lasimulación
